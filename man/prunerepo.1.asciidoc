...
-------
--days DAYS::
    only remove packages (and build directories when --cleancopr is used) that are DAYS old or older
    (for packages by their build date, for directories the last modification time is considered)
