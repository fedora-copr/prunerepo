...
function listpkgsbyrepo {
	dnf repoquery --repofrompath=test_prunerepo,file://$testrepo --setopt=cachedir=prunerepo-tests-dnf-cache --repo=test_prunerepo --refresh --quiet --location --setopt='skip_if_unavailable=False' | sed 's|file://||' | sort
}
